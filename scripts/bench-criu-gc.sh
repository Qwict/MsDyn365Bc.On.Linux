...
#
# The hypothesis this was built to test — that criu restore time tracks the
# NUMBER of memory mappings rather than the bytes — is WRONG, and this sweep is
# what disproved it. Cutting the heaps moves the mapping count ~10%
# (5,645 -> 5,138) while the checkpoint halves (3.0G -> 1.5G) and criu halves
# with it (26s -> 12s). The variable is bytes.
#
# It is kept because it is the harness that measures the TRADE. Server GC is
# the default because it speeds up the parallel Roslyn compile during startup
# and extension publish, so a configuration that restores 14s faster is only a
# win if it does not cost that back during the run. This sweep measures the
# restore side; the other side needs test wall clock on a real suite.
#
